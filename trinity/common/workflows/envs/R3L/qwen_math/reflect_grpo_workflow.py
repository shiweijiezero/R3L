...
@WORKFLOWS.register_module("reflect_grpo_qwen_math_workflow")
class ReflectGRPOQwenMathWorkflow(Workflow):
    """
    Reflect GRPO Workflow for Qwen Math problem solving.
    ...
    def __init__(
        ...
        self.data_dir = "reflect_grpo_qwen_math_data"
        ...
        print(f"Initializing ReflectGRPOQwenMathWorkflow, temperature={self.temperature}")
        self.reset(task)
    ...
    def get_reflect(self, trajectory):
        ...
        try:
            ...
            print(f"[ReflectGRPO-Qwen] Reflection failed - Error: {str(e)}")
            return None, None
    ...
    def run(self) -> List[Experience]:
        ...
        for i in range(self.n // 2):
            try:
                ...
                print(
                    f"[ReflectGRPO-Qwen] Rollout {i} - reward: {reward}, attempts: {attempts}"
                )
                ...
                print(f"[ReflectGRPO-Qwen] Rollout {i} failed - Error: {str(e)}")
        ...
        for i in range(self.n // 2, self.n):
            try:
                ...
                print(
                    f"[ReflectGRPO-Qwen] First attempt {i} - reward: {reward}, attempts: {attempts}"
                )
                ...
                if not success:
                    print(
                        f"[ReflectGRPO-Qwen] Failed attempt {i}, attempting reflection and retry from beginning..."
                    )
                    ...
                    if is_valid and not is_perfect:
                        print(
                            f"[ReflectGRPO-Qwen] Valid reflection generated, retrying from the beginning..."
                        )
                        guidance_prompt = utils.reflect_report_to_guidance_prompt(
                            reflect_checklist
                        )
                        ...
                        try:
                            ...
                            print(
                                f"[ReflectGRPO-Qwen] Retry {i} - reward: {second_reward}, attempts: {second_attempts}, improved: {second_reward > reward}"
                            )
                            ...
                            print(
                                f"[ReflectGRPO-Qwen] Retry {i} failed - Error: {str(e)}"
                            )
                    else:
                        if is_perfect:
                            print(
                                f"[ReflectGRPO-Qwen] Reflection indicates perfect attempt - No retry needed"
                            )
                        else:
                            print(
                                f"[ReflectGRPO-Qwen] Invalid reflection - Skipping retry"
                            )

            except Exception as e:
                print(f"[ReflectGRPO-Qwen] Attempt {i} failed - Error: {str(e)}")
