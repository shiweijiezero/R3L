# -*- coding: utf-8 -*-
"""Step-level Critique-GRPO workflow for WebShop with Qwen3 compatibility."""
...
@WORKFLOWS.register_module("step_critique_grpo_webshop_workflow")
class StepLevelCritiqueGRPOWebshopWorkflow(StepLevelWebshopBaseWorkflow):
    """Step-level Critique-GRPO for WebShop.
    ...
    def _generate_critique(self, trajectory, reward) -> Tuple[Optional[str], Optional[Any]]:
        ...
        try:
            ...
            print(f"[StepCritiqueGRPO-WebShop] Critique generation failed: {e}")
    ...
    def _run_refinement_rollout(self, critique_text, run_id):
        ...
        try:
            exps = super(StepLevelWebshopBaseWorkflow, self).run()
        except Exception as e:
            print(f"[StepCritiqueGRPO-WebShop] Refinement rollout failed: {e}")
            return [], -0.1
    ...
    def run(self) -> List[Experience]:
        ...
        for i in range(self.n):
            ...
            if run_exps:
                ...
                print(f"[StepCritiqueGRPO-WebShop] Initial {i+1}/{self.n} - reward: {reward}")
        ...
        for i, result in enumerate(initial_results):
            ...
            if ref_exps and ref_reward > result["reward"]:
                ...
                print(
                    f"[StepCritiqueGRPO-WebShop] Refinement for initial {i+1} - "
                    f"reward: {ref_reward}"
                )
        ...
        print(
            f"[StepCritiqueGRPO-WebShop] {len(exp_lst)} step-experiences, "
