# -*- coding: utf-8 -*-
"""Step-level Critique-GRPO workflow for DAPO math with Qwen3 compatibility."""
...
@WORKFLOWS.register_module("step_critique_grpo_dapo_workflow")
class StepLevelCritiqueGRPODapoWorkflow(StepLevelDapoBaseWorkflow):
    """Step-level Critique-GRPO for DAPO.

    1. Generate N initial rollouts (step-level)
    2. For each failed rollout, generate critique + refinement (retry with guidance)
    ...
    def _generate_critique(self, trajectory, reward) -> Tuple[Optional[str], Optional[Any]]:
        ...
        try:
            ...
            print(f"[StepCritiqueGRPO-DAPO] Critique generation failed: {e}")
    ...
    def _run_refinement_rollout(self, critique_text, run_id):
        ...
        guidance = (
            ...
            f"Focus on correct mathematical reasoning."
        )
        self.memory[0] = {
            "role": "system",
        ...
        try:
            ...
            print(f"[StepCritiqueGRPO-DAPO] Refinement rollout failed: {e}")
            return [], 0.0
    ...
    def run(self) -> List[Experience]:
        ...
        for i in range(self.n):
            ...
            if run_exps:
                ...
                print(f"[StepCritiqueGRPO-DAPO] Initial {i+1}/{self.n} - reward: {reward}")
        ...
        for i, result in enumerate(initial_results):
            ...
            if ref_exps and ref_reward > result["reward"]:
                ...
                print(
                    f"[StepCritiqueGRPO-DAPO] Refinement for initial {i+1} - "
                    f"reward: {ref_reward}"
                )
        ...
        print(
            f"[StepCritiqueGRPO-DAPO] {len(exp_lst)} step-experiences, "
