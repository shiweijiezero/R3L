...
@ALGORITHM_TYPE.register_module("step_opmd")
class StepOPMDAlgorithm(AlgorithmType):
    """Step-level OPMD Algorithm.

    Uses step-wise OPMD advantage (mean baseline, no std normalization)
    with OPMD policy loss for step-level training.
    """
...
@ALGORITHM_TYPE.register_module("step_opmd_reweight_adv")
class StepOPMDReweightAdvAlgorithm(AlgorithmType):
    """Step-level OPMD with reweighting advantage algorithm.

    Used for step-level R3L training where each turn is an independent experience.
    Combines step-wise advantage broadcasting with OPMD reweighting (positive amplification).
    """
    ...
    def default_config(cls) -> Dict:
        return {
            "repeat_times": 8,
            "advantage_fn": "step_wise_opmd_reweight_adv",
            "sample_strategy": "default",
            "policy_loss_fn": "opmd",
            "kl_penalty_fn": "none",
            "kl_loss_fn": "none",
