...
@WORKFLOWS.register_module("R3L_qwen_math_workflow")
class R3LQwenMathWorkflow(Workflow):
    """
    R3L workflow for Qwen Math problem solving
    """
    ...
    def __init__(
        ...
        print(
            f"Initializing R3LQwenMathWorkflow with experience learning, temperature={self.temperature}"
        )
        self.reset(task)
    ...
    def get_reflect(
        ...
        try:
            ...
            print(f"[R3L-Qwen] Reflection failed - Error: {str(e)}")
    ...
    def run(self) -> List[Experience]:
        """Run the R3L Qwen Math workflow and return experiences"""
        ...
        for i in range(self.n // 2):  # Half for rollout, half for reflection + retry
            try:
                ...
                print(f"[R3L-Qwen] First rollout - reward: {reward}, attempts: {attempts}")
                ...
                print(
                    f"[R3L-Qwen] Starting reflection on first attempt (reward: {reward})..."
                )
                ...
                if reflect_checklist is None:
                    print(
                        f"[R3L-Qwen] Reflection failed - No valid reflection data generated"
                    )
                elif is_valid and not is_perfect:
                    print(f"[R3L-Qwen] Reflection successful - Valid reflection generated")
                elif is_perfect:
                    print(
                        f"[R3L-Qwen] Reflection indicates perfect first attempt - No retry needed"
                    )
                elif not is_valid:
                    print(
                        f"[R3L-Qwen] Reflection validation failed - Invalid reflection data"
                    )
                ...
                if not is_valid or is_perfect:
                    ...
                    print(
                        "[R3L-Qwen] Valid reflection obtained, proceeding to second rollout..."
                    )
                    ...
                    print(f"[R3L-Qwen] Pivot point (retry_from_step): {retry_step}")
                    ...
                    try:
                        ...
                        print(
                            f"[R3L-Qwen] Second rollout - reward: {second_reward}, attempts: {second_attempts}, improve: {second_reward > reward}"
                        )
                        ...
                        if retry_step > 0:
                            ...
                            for existing_exp in exp_lst:
                                if existing_exp.eid.run == exp_run_id:
                                    self._adjust_action_mask_for_retry(
                                        existing_exp, retry_step
                                    )
                                    break
                        ...
                        if second_reward > reward and second_reward >= 1.0:
                            print(
                                f"[R3L-Qwen] Second attempt successful improvement - Recording reflection and retry experiences"
                            )
                            print(
                                f"[R3L-Qwen] Reward improvement: {reward} -> {second_reward} (+{second_reward - reward:.2f})"
                            )
                            reflect_exp.reward = 1.0
                            # Set eid
                            ...
                            print(
                                "[R3L-Qwen] Reflection and retry led to improvement, recording both..."
                            )
                        elif second_reward <= reward:
                            print(
                                f"[R3L-Qwen] Second attempt did not improve - First reward: {reward}, Second reward: {second_reward}"
                            )
                        else:
                            print(
                                f"[R3L-Qwen] Second attempt improved but below threshold - Reward: {second_reward} (need >= 1.0)"
                            )
                    except Exception as e:
                        print(f"[R3L-Qwen] Second rollout failed - Error: {str(e)}")
            except Exception as e:
                print(f"[R3L-Qwen] Rollout iteration {i} failed - Error: {str(e)}")

        # Print summary statistics
        print(f"\n[R3L-Qwen Summary] Generated {len(exp_lst)} experiences")
        total_reward = sum(exp.reward for exp in exp_lst)
        avg_reward = total_reward / len(exp_lst) if exp_lst else 0.0
        print(
            f"[R3L-Qwen Summary] Total reward: {total_reward:.2f}, Average reward: {avg_reward:.2f}"
        )
